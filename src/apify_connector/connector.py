...
from .models import (
    ListingDetails, ListingBasic, CalendarDay, Calendar,
    Review, SearchResult, Host, Location, Rating,
)
from .exceptions import (
    ApifyError, ActorRunError, RateLimitError,
    AuthenticationError, DataParsingError, ListingNotFoundError, NoResultsError,
)
...
class ApifyConnector:
    ...
    def __init__(self, api_token: str, timeout: int = DEFAULT_TIMEOUT, max_retries: int = 3):
    ...
    async def _run_actor(self, actor_id: str, input_data: Dict[str, Any], wait_for_finish: bool = True) -> List[Dict[str, Any]]:
        url = f"{self.BASE_URL}/acts/{actor_id}/runs"
        logger.info(f"Running Actor: {actor_id}")
        
        for attempt in range(self.max_retries):
            try:
                response = await self.client.post(url, json=input_data, params={"waitForFinish": self.timeout if wait_for_finish else 0})
                ...
                if response.status_code >= 400:
                    raise ActorRunError(actor_id=actor_id, message=f"HTTP {response.status_code}: {response.text}")
                ...
                if status == "SUCCEEDED":
                    ...
                    raise ActorRunError(actor_id=actor_id, message="Actor run failed", run_id=run_data["id"])
                elif status == "RUNNING" and wait_for_finish:
                    return await self._wait_for_run(actor_id, run_data["id"])
                return []
    ...
    async def _wait_for_run(self, actor_id: str, run_id: str, poll_interval: int = 5) -> List[Dict[str, Any]]:
    ...
    async def get_listing(self, listing_id: str) -> ListingDetails:
        ...
        results = await self._run_actor(ApifyActors.LISTING_SCRAPER, {
            "listingIds": [listing_id], "maxListings": 1,
            "includeReviews": False, "includeCalendar": False,
        })
    ...
    async def get_listings(self, listing_ids: List[str]) -> List[ListingDetails]:
        ...
        results = await self._run_actor(ApifyActors.LISTING_SCRAPER, {
            "listingIds": ids, "maxListings": len(ids),
            "includeReviews": False, "includeCalendar": False,
        })
        return [self._parse_listing_details(r) for r in results]

    async def search_by_location(self, latitude: float, longitude: float, radius_km: float = 2.0,
                                  max_results: int = 50, room_type: Optional[str] = None,
                                  min_price: Optional[float] = None, max_price: Optional[float] = None,
                                  min_bedrooms: Optional[int] = None, currency: str = "EUR") -> SearchResult:
        logger.info(f"Searching listings at ({latitude}, {longitude}), radius={radius_km}km")
        
        input_data = {"locationQuery": f"{latitude},{longitude}", "maxListings": max_results, "currency": currency, "simple": True}
        if room_type: input_data["roomType"] = room_type
        if min_price is not None: input_data["minPrice"] = int(min_price)
        if max_price is not None: input_data["maxPrice"] = int(max_price)
        if min_bedrooms is not None: input_data["minBedrooms"] = min_bedrooms
        ...
        for r in results:
            ...
            if listing.location:
                distance = self._haversine_distance(latitude, longitude, listing.location.latitude, listing.location.longitude)
        ...
        return SearchResult(
            listings=listings[:max_results], total_count=len(listings),
            location=Location(latitude=latitude, longitude=longitude),
            search_radius_km=radius_km, currency=currency,
        )

    async def search_by_address(self, address: str, radius_km: float = 2.0, max_results: int = 50, **kwargs) -> SearchResult:
        ...
        return SearchResult(
            listings=listings[:max_results], total_count=len(listings),
            location=center, search_radius_km=radius_km, currency=kwargs.get("currency", "EUR"),
        )
    ...
    async def get_calendar(self, listing_id: str, days: int = 365) -> Calendar:
        ...
        results = await self._run_actor(ApifyActors.CALENDAR_SCRAPER, {
    ...
    async def get_reviews(self, listing_id: str, limit: int = 100) -> List[Review]:
        ...
        results = await self._run_actor(ApifyActors.REVIEWS_SCRAPER, {
    ...
    def _parse_listing_details(self, data: Dict[str, Any]) -> ListingDetails:
        try:
            ...
            if data.get("lat") and data.get("lng"):
                location = Location(latitude=data["lat"], longitude=data["lng"], city=data.get("city"), neighborhood=data.get("neighborhood"), country=data.get("country"))
            
            host = None
            host_data = data.get("host", {})
            if host_data:
                host = Host(id=str(host_data.get("id", "")), name=host_data.get("name", ""), is_superhost=host_data.get("isSuperhost", False), profile_pic_url=host_data.get("profilePicUrl"), response_rate=host_data.get("responseRate"), response_time=host_data.get("responseTime"))
            
            rating_details = None
            if data.get("rating"):
                rating_details = Rating(overall=data.get("rating", 0), accuracy=data.get("ratingAccuracy"), cleanliness=data.get("ratingCleanliness"), checkin=data.get("ratingCheckin"), communication=data.get("ratingCommunication"), location=data.get("ratingLocation"), value=data.get("ratingValue"))
    ...
    def _parse_listing_basic(self, data: Dict[str, Any]) -> ListingBasic:
        location = None
        if data.get("lat") and data.get("lng"):
            location = Location(latitude=data["lat"], longitude=data["lng"], city=data.get("city"), neighborhood=data.get("neighborhood"))
    ...
    def _parse_calendar(self, listing_id: str, results: List[Dict[str, Any]]) -> Calendar:
        ...
        return Calendar(listing_id=listing_id, days=days, currency=results[0].get("currency", "EUR") if results else "EUR")
    ...
    def _extract_listing_id(self, listing_input: str) -> str:
        ...
        patterns = [r"airbnb\.[a-z.]+/rooms/(\d+)", r"airbnb\.[a-z.]+/h/([a-zA-Z0-9-]+)", r"/rooms/(\d+)", r"listing[_-]?id[=:]?\s*(\d+)"]
